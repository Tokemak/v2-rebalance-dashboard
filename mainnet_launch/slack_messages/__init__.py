...
def post_daily_messages(slack_channel: SlackChannel = SlackChannel.PRODUCTION):
    ...
    for fn in [post_autopools_without_generated_plans, post_missing_balance_updated_events, post_unsold_incentive_tokens, post_asset_depeg_slack_message]:
...
def post_weekly_messages(slack_channel: SlackChannel = SlackChannel.PRODUCTION):
    ...
    for fn in [post_destination_ownership_exposure_table, post_illiquid_token_holding_analysis, post_possible_new_destinations]:
        post_message_or_error_message(fn, slack_channel)
