...
def _make_all_destination_composite_return_df(autopool: AutopoolConstants) -> go.Figure:
    compositeReturn_out_df = 100 * fetch_destination_summary_stats(autopool, "total_apr_out")
    pricePerShare_df = 100 * fetch_destination_summary_stats(autopool, "pool_safe_price")
    ownedShares_df = 100 * fetch_destination_summary_stats(autopool, "autopool_owned_shares")
    ...
    composite_return_fig.update_layout(yaxis_title="Composite Return (out) (%)")
...
if __name__ == "__main__":
    ...
    fetch_and_render_weighted_crm_data(AUTO_ETH)
