...
They answer in a determanistic way:
...
"What was the price return for this destination between these two blocks?"

# this is for establishing ground truth. Later on this info can be used to make predictions. 
...
def get_seconds_between_blocks(start_block: int, end_block: int, chain: ChainData) -> int:
    """
    Returns the number of seconds between two blocks.
    This should be implemented based on the specific blockchain's block time.
    """
    # include checkes that this works for every chain
...
class BacktestBaseAndFeeAPRCalculator(ABC):
    ...
    def compute_base_apr(self, start_block: int, end_block: int) -> float:
        ...
        start_rate = Decimal(str(self._get_rate(start_block)))
        end_rate = Decimal(str(self._get_rate(end_block)))
        rate_delta = end_rate - start_rate

        seconds_between_blocks = get_seconds_between_blocks(start_block, end_block)
        portion_of_year = Decimal(seconds_between_blocks) / Decimal(SECONDS_PER_YEAR)
        base_apr = float((rate_delta / start_rate) / portion_of_year)
...
class PriceReturnCalculator(ABC):
    ...
    def _get_price(self, block: int) -> float:
        """
        get the "safe" or "spot" up to your discretion price of 1 LP token at a given block.
        """
        pass
