...
def render_filters(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Pull (or init) current selections from session_state
    ss = st.session_state
    ...
    # ---- UI (cascading) ----
    # Step 1: sell_amount_norm options from full df
    sell_amt_opts = ["All"] + uniq_vals(df, "sell_amount_norm")

    # Coerce stored choice in case it became invalid
    ...
    # Apply first-level mask
    mask = pd.Series(True, index=df.index)
    if sell_amt_choice != "All":
        mask &= df["sell_amount_norm"] == sell_amt_choice
    df_lvl1 = df[mask]

    # Step 2: buy_symbol options from df_lvl1
    buy_symbol_opts = uniq_vals(df_lvl1, "buy_symbol")
    ...
    # Apply second-level mask
    if buy_syms:
        mask &= df["buy_symbol"].isin(buy_syms)
    df_lvl2 = df[mask]

    # Step 3: sell_symbol options from df_lvl2
    sell_symbol_opts = uniq_vals(df_lvl2, "sell_symbol")
    ...
    # Apply third-level mask
    if sell_syms:
        mask &= df["sell_symbol"].isin(sell_syms)
    df_lvl3 = df[mask]

    # Step 4: autopool_name options from df_lvl3
    if "autopool_name" in df.columns:
        autopool_opts = uniq_vals(df_lvl3, "autopool_name")
    else:
        autopool_opts = []
...
def _build_diff_df(_df: pd.DataFrame, exclude_threshold: float, minutes: str, max_lag: int) -> pd.DataFrame:
    ...
    # total_slippage_values = int((~n_minutes_slippage_values.isna()).sum().sum())
    total_non_outlier_slippage_values = int((~non_outlier_hourly_slippage_values.isna()).sum().sum())
    # print(
    #     f"Using {total_non_outlier_slippage_values=}/{total_slippage_values=} total values {100*total_non_outlier_slippage_values/total_slippage_values:.1f}% after excluding anything above {exclude_threshold} bps slippage"
    # )
    diffs = []
...
def _make_ecdf_slippage_diff_plot(
    ...
    fig.add_vline(x=-1, line_dash="dash", line_color="red", sannotation_text="-1 bps", annotation_position="top left")
    return fig
...
def main():
    ...
    fig = _make_ecdf_slippage_diff_plot(
        diff_df,
