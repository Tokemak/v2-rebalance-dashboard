...
async def safe_get_raw_state_by_block_one_block(calls: list[Call], block: int):
    multicall = Multicall(
        calls=calls, block_id=block, _w3=eth_client, require_success=False
    )
    response = await multicall.coroutine()
    return response


def _identity_with_bool_success(success, value):
    if success:
        return value
    return None
...
def _build_default_block_and_timestamp_calls():
    ...
    get_block_call = Call(
        ...
        [("block", _identity_with_bool_success)],
    )

    get_timestamp_call = Call(
        multicall_v3,
        ["getCurrentBlockTimestamp()(uint256)"],
        [("timestamp", _identity_with_bool_success)],
    )
    return get_block_call, get_timestamp_call


def _data_fetch_builder(
    semaphore: asyncio.Semaphore, responses: list, failed_multicalls: list
):
...
async def safe_get_raw_state_by_block(
    ...
    for semaphore_limit in semaphore_limits:
        ...
        failed_multicalls = []
        _ratelimited_async_data_fetcher = _data_fetch_builder(
            semaphore, responses, failed_multicalls
        )
        await asyncio.gather(
            *[_ratelimited_async_data_fetcher(m) for m in calls_remaining]
        )
